...
export default function ProfiloPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(t('Errore nel caricamento dei dati'))
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleCancelBooking = async (bookingId: string) => {
    if (!confirm(t('Vuoi annullare questa prenotazione?'))) return
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || t('Errore nella cancellazione'))
      }
      // Re-fetch bookings from server to ensure data is fresh
      await loadBookings()
      setSuccess(t('Prenotazione annullata con successo'))
      setTimeout(() => setSuccess(''), 4000)
    } catch (err) {
      setError(err instanceof Error ? err.message : t('Errore nella cancellazione'))
    } finally {
      setCancelling(null)
    }
  }
  ...
  const formatDate = (isoStr: string) => {
    ...
    return d.toLocaleDateString(isEn ? 'en-GB' : 'it-IT', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
  }

  const formatTime = (isoStr: string) => {
    const d = new Date(isoStr)
    return d.toLocaleTimeString(isEn ? 'en-GB' : 'it-IT', { hour: '2-digit', minute: '2-digit' })
  }
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700 p-6 shadow-sm mb-6">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                <h1 className="text-xl font-semibold text-stone-900 dark:text-stone-100">{customer?.nome || t('Cliente')}</h1>
                <p className="text-sm text-stone-500 dark:text-stone-400">{t('Il tuo profilo')}</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-stone-700 dark:text-stone-300">
                  {customer?.telefono?.startsWith('temp_') ? t('Non impostato') : (customer?.telefono || '—')}
                </span>
              </div>
            </div>
          ...
          <div className="mb-6">
            ...
            {futureBookings.length === 0 ? (
              <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700 p-8 text-center">
                ...
                <p className="text-sm text-stone-400 dark:text-stone-500">{t('Nessun appuntamento programmato')}</p>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {booking.services.map((s, i) => (
                        <span key={i} className="px-2.5 py-1 rounded-lg bg-stone-50 dark:bg-stone-800/50 text-stone-600 dark:text-stone-400 text-xs">
                          {s.name} ({s.durationMinutes} {t('min')})
                        </span>
                      ))}
                    </div>
                  ...
                  <div key={booking.id} className="bg-white dark:bg-stone-900 rounded-xl border border-stone-100 dark:border-stone-800 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        {booking.status === 'cancelled' ? t('Annullato') : t('Completato')}
                      </span>
                    </div>
                  </div>
                ))}
